            ...
            if (!longFormat)
            {
                ...
                switch (timeComponents)
                {
                    ...
                    case TimeComponents.Minutes | TimeComponents.Milliseconds:
                        throw ErrorFactory.ArgumentException("Time Components are invalid. They cannot be only " + timeComponents.ToString() + " and in a short format", nameof(timeComponents)).Create();
                }
            }
